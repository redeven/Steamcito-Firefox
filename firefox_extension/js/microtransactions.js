...
let otrosContenedores = document.querySelectorAll(
  '.approvetxn_lineitem_row .approvetxn_lineitem_price,#review_subtotal_value,#review_taxes_value'
);
...
function setearPrecios() {
  if (enlace.href.indexOf('addfunds') == -1) {
    ...
    contenedorPrecioFinal.innerHTML = DOMPurify.sanitize(
      argentinizar(calcularImpuestos(stringToNumber(contenedorPrecioFinal)))
    );
    otrosContenedores.forEach((contenedor) => {
      if (contenedor.innerHTML.indexOf('ARS$') != -1)
        contenedor.innerText = DOMPurify.sanitize(argentinizar(calcularImpuestos(stringToNumber(contenedor))));
    });
  }
}
