...
function setTransactionType(transactions) {
  transactions.forEach((transaction) => {
    ...
    if (payments.length) {
      ...
      contenedorTotal.innerHTML += DOMPurify.sanitize(
        `<b>(Precio Steam)</b> <br><br> ${steamizar(stringToNumber(walletValue))} <br> ${argentinizar(calcularImpuestosHistorial(stringToNumber(ccValue), date))}`
      );
    }
  ...
  standardTaxesDetail.forEach((tax) => {
    ...
    tax.values.forEach((value) => {
      if (
        ...
        (date.year == value.year && (date.month > value.month || (date.month == value.month && date.day >= value.day)))
      ) {
...
function checkForReload(pickedYear) {
  const lastDate = document.querySelector(
    '.wallet_history_table tbody > tr.wallet_table_row:nth-last-child(2) td[class*=date]'
  ).innerText;
...
function showCalculoHtml(pickedYear) {
  ...
  let transactionElements = Array.from(
    document.querySelectorAll('.split-purchase:not(.picked),.cc-purchase:not(.picked)')
  );
...
const showDevolucionHtml = () => {
  const html = `<div class="aviso-devolucion">
        <div class="left">
