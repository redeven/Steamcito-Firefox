...
const getAppPricing = async (appInitialData) => {
  ...
  const appIdFetch = await fetch(`${type == 'app' ? `${appEndpoint}&cc=us` : `${subEndpoint}&cc=us`}`, {
    credentials: 'omit',
  });

  const appIdFetchArg = await fetch(`${type == 'app' ? `${appEndpoint}&cc=ar` : `${subEndpoint}&cc=ar`}`, {
    credentials: 'omit',
  });
  ...
  if (appIdResponse[id].success && appIdArgResponse[id].success) {
    ...
    const nearestOption = regionalPricingOptionsLatam.reduce((prev, curr) =>
      Math.abs(curr - appData.baseUsdPrice) < Math.abs(prev - appData.baseUsdPrice) ? curr : prev
    );
    ...
    const recommendedArsPrice =
      (regionalPricingChartLatam.filter((item) => item.usdPrice == nearestOption).map((item) => item.argPrice)[0] *
        (100 - appData.discount)) /
      100;
    ...
    const pppPrice =
      (regionalPricingChartLatamPPP.filter((item) => item.usdPrice == nearestOption).map((item) => item.argPrice)[0] *
        (100 - appData.discount)) /
      100;
    ...
    if (appData.arsPrice > appData.recommendedArsPrice && appData.arsPrice != appData.usdPrice) {
      appData.regionalDifference = Math.round(
        (parseFloat(appData.arsPrice - appData.recommendedArsPrice) / appData.recommendedArsPrice) * 100
      );
      ...
      appData.regionalDifference = Math.round(
        (parseFloat(appData.recommendedArsPrice - appData.arsPrice) / appData.recommendedArsPrice) * 100
      );
...
const renderCryptoPrice = async (appData) => {
  ...
  let CryptoPriceContainer = `<a class="steamcito_saving_tip_url" href="https://steamcito.com.ar/mejor-metodo-de-pago-steam-argentina" target="_blank">
  ...
  if (savingTipCloseButton) {
    savingTipCloseButton.addEventListener('click', (e) => {
      ...
      savingTipElement &&
        savingTipElement.insertAdjacentHTML(
          'beforebegin',
          DOMPurify.sanitize(
            '<span>Podés habilitar la opción nuevamente desde el Menú de Opciones -> Tips de Ahorro</span><br><br>'
          )
        );
...
const renderExchangeIndicator = (
  ...
  dolarTarjetaItem &&
    dolarTarjetaItem.addEventListener('click', () => {
      changePaymentMethodState('steamcito-cotizacion-tarjeta');
      window.location.reload();
    });

  dolarCryptoItem &&
    dolarCryptoItem.addEventListener('click', () => {
      changePaymentMethodState('steamcito-cotizacion-crypto');
      window.location.reload();
    });

  dolarMepItem &&
    dolarMepItem.addEventListener('click', () => {
      changePaymentMethodState('steamcito-cotizacion-mep');
      window.location.reload();
    });
};
