...
function renderCart() {
  ...
  if (cartSidebar && cartContent) {
    let total = Array.from(cartSidebar.querySelectorAll('div:not(:has(*))')).find(
      (element) => element.innerText[0] == '$'
    );
    if (total?.innerText) {
      ...
      if (!document.querySelector('.steamcito_cart')) {
        cartSidebar.insertAdjacentHTML(
          'beforebegin',
          DOMPurify.sanitize(`
...
async function getArgentinaGames() {
  ...
  if (shouldRefreshArgentinaList) {
    try {
      let argentinaGames = await fetch(
        'https://store.steampowered.com/curator/45013169/ajaxgetfilteredrecommendations/?query&start=0&count=1000'
      );
...
function renderPrices(price) {
  ...
  if (walletBalance > parseFloat(price.dataset.originalPrice)) {
    ...
    if (price.previousElementSibling) {
      if (isInsideString(price.previousElementSibling, '$')) {
        ...
        price.previousElementSibling.innerText = DOMPurify.sanitize(
          numberToStringUsd(price.previousElementSibling.dataset.originalPrice)
        );
      }
    }
  }
  ...
  // Si el saldo no alcanza
    ...
    if (price.previousElementSibling) {
      if (isInsideString(price.previousElementSibling, '$')) {
        ...
        price.previousElementSibling.innerText = DOMPurify.sanitize(
          numberToString(price.previousElementSibling.dataset.argentinaPrice)
        );
      }
    }
  }
  ...
  setTimeout(function () {
    if (
      price.classList.contains('argentina') &&
      !price.innerText.includes('ARS') &&
      (price.closest('.dynamic_bundle_description') || price.closest('div[data-bundlediscount'))
    ) {
      setArgentinaPrice(price);
    }

    if (
      price.classList.contains('original') &&
      !price.innerText.includes('USD') &&
      price.closest('.dynamic_bundle_description')
    ) {
      setArgentinaPrice(price);
    }
  }, 1500);
...
function switchPrices(selector, first, second, symbol) {
  setTimeout(function () {
    ...
    if (selector.classList.contains('suscription-price')) {
      ...
      selector.innerHTML =
        first == 'argentina'
          ? DOMPurify.sanitize(numberToStringUsd(selector.dataset[second + 'Price'] + symbol))
          : DOMPurify.sanitize(numberToString(selector.dataset[second + 'Price'] + symbol));
    }
  }, 250);
}
...
async function processExchangeRate(type, localStorageItemKey, defaultValue) {
  try {
    let exchangeRateResponse = await fetch(
    ...
    if (exchangeRateJson.results_html) {
      ...
      if (gamesElements.length) {
        ...
        const formattedDate = new Date(parseInt(rateData[3]))
...
function monthStrToNumber(month) {
  if (month.indexOf(',')) {
    ...
    return (
      ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC'].indexOf(monthSanitized) + 1
    );
  } else {
