let precios = document.querySelectorAll(
  '.updateSubscriptionOptionPrice,.transactionRowAmountDue,.itemSubtext,.game_area_purchase_game_dropdown_menu_item_text,.game_area_purchase_game_dropdown_selection span'
);
  ...
  if (element.innerText.indexOf('$', start) != -1) {
    ...
    element.innerHTML = DOMPurify.sanitize(
      element.innerHTML.replace(
        numeroOriginal,
        `<span class="suscription-price" data-argentina-price="${numeroArgentino}" data-original-price="${numeroOriginal}">${numeroOriginal}</span>`
      )
    );
